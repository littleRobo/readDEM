...
public:
    colormap();
    colormap(Mat img){this->data = img.clone();}//CV_32FC1
    ...
    void findMaxMin(float &max, float &min){
        ...
        for(int i=0; i<this->data.rows; i++){
            for(int j=0; j<this->data.cols; j++){
                float value;
                value = this->data.at<float>(i,j);
                if(value > lowbound && value < upbound){
                    if(min>value) min=value;//surpose bgcolor < -1e4
    ...
    void showColormap(){
        ...
        float upbound = UP_BOUND;
        float lowbound = LOW_BOUND;
        Mat img(this->data.rows, this->data.cols, CV_8UC3);
        for(int i=0; i<this->data.rows; i++){
            for(int j=0; j<this->data.cols; j++){
                float value = this->data.at<float>(i,j);
                if(value > lowbound && value < upbound){
                    int index = round((value-min)/(max-min)*255);
                    img.at<Vec3b>(i,j)[0] = turbo_srgb_bytes[index][2];
                    img.at<Vec3b>(i,j)[1] = turbo_srgb_bytes[index][1];
                    img.at<Vec3b>(i,j)[2] = turbo_srgb_bytes[index][0];
                }
                else{
        ...
        namedWindow("colormap",0);
        resizeWindow("colormap", 512,512);
        imshow("colormap", img);
    }
};
